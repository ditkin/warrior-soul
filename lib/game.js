...
class Game {
  ...
  spawnEntity(type, x, y, settings) {
    var EntityClass = typeof type === 'string' ? window.igEntities[type] : type

    if (!EntityClass) {
      throw new Error('Cant spawn entity of type ' + type)
    }
  ...
  draw() {
    ...
    this._rscreen.x =
      IG.instance.system.getDrawPos(this.screen.x) / IG.instance.system.scale
    this._rscreen.y =
      IG.instance.system.getDrawPos(this.screen.y) / IG.instance.system.scale
